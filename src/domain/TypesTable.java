...
public class TypesTable {
	...
	private static Map<String, List<String>> disadvantage; // Tipos supereficaces contra este

	private static Map<String, List<String>> advantage;	// Tipos débiles contra este

	private static Map<String, List<String>> inmunity;	// Tipos inmunes a este
	 ...
	 */
	public TypesTable() {
		disadvantage = new TreeMap<>();
		advantage = new TreeMap<>();
		inmunity = new TreeMap<>();
		setUpTables();
	...
	public static int calculatePhysicalDamage(Pokemon user, Pokemon target, Move move, BattleField field) {
		...
		double attackerAttack = user.getRealAttack();
		int attackerPower = move.getPower();
		double targetDefense = target.getRealDefense();
		double typeBonification = user.getTypes().contains(move.getType()) ? 1.5 : 1;
		double effectivity = getEffectivityMultiplier(move,target);
		int variation = rand.nextInt(85,101);
		double weather = getWeatherMultiplier(move,field);

		double damage = weather * 0.01 * typeBonification * effectivity * variation * (((((0.2 * level)+ 1)* attackerAttack * attackerPower)/(25 * targetDefense))+ 2);
		return (int)damage;
	}
	/**
	...
	public static int calculateSpecialDamage(Pokemon user, Pokemon target, Move move, BattleField field) {
		Random rand = new Random();
		int level = user.getLevel();
		double attackerAttack = user.getRealSpecialAttack();
		int attackerPower = move.getPower();
		double targetDefense = target.getRealSpecialDefense();
		double typeBonification = user.getTypes().contains(move.getType()) ? 1.5 : 1;
		double effectivity = getEffectivityMultiplier(move,target);
		int variation = rand.nextInt(85,101);
		double weather = getWeatherMultiplier(move,field);
	...
	public static double getEffectivityMultiplier(Move move, Pokemon target) {
		...
		for (String t : target.getTypes()){
			if (inmunity.get(t).contains(move.getType())){
				e = 0;
			}
			else{
				if (disadvantage.get(t).contains(move.getType())){
					e = e*2;
				}
				else if(advantage.get(t).contains(move.getType())){
					e = e*0.5;
				}
			}
		}
